...
def drawLinesOnImage(f1, f2, cluster_img):
    ...
    for i in range(0, len(f1), 2):
        cv2.line(img, (int(f1[i].pt[0]), int(f1[i].pt[1])), (int(f2[i].pt[0]), int(f2[i].pt[1])), (0, 0, 255), 1,
                 cv2.LINE_AA)
...
def main(file_name):
    ...
    output, flag = drawLinesOnImage(f1, f2, cluster_img)
    ...
    DEBUG.printFile()
    DEBUG.printImage()
    ima = DEBUG.getClusterImage()
    print(dt.datetime.now())
    return ima, output, flag
...
class Detection:
    ...
    def Detector(self):
        ...
        print(tail)
        file_name = []
        file_name.append(os.path.abspath(self.path))
        file_name.append(tail)
        print(file_name)
        ima, output, flag = main(file_name)
        return ima, output, flag
